<?xml version="1.0" encoding="UTF-8" standalone="yes"?>
<gxl xmlns="http://www.gupro.de/GXL/gxl-1.0.dtd">
    <graph role="graph" edgeids="false" edgemode="directed" id="stmhaskell_o_cg.s1125">
        <attr name="$version">
            <string>curly</string>
        </attr>
        <node id="n753">
            <attr name="layout">
                <string>123 8 67 126</string>
            </attr>
        </node>
        <node id="n846">
            <attr name="layout">
                <string>50 190 67 108</string>
            </attr>
        </node>
        <node id="n878">
            <attr name="layout">
                <string>198 190 66 90</string>
            </attr>
        </node>
        <node id="n908">
            <attr name="layout">
                <string>51 354 66 108</string>
            </attr>
        </node>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>type:Tr</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>flag:comm</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>flag:done</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>flag:loop_s</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>let:id = int:0</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>let:writes = string:"x"</string>
            </attr>
        </edge>
        <edge from="n753" to="n753">
            <attr name="label">
                <string>let:writes = string:"y"</string>
            </attr>
        </edge>
        <edge from="n753" to="n878">
            <attr name="label">
                <string>rf</string>
            </attr>
        </edge>
        <edge from="n753" to="n846">
            <attr name="label">
                <string>ww</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>type:Tr</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>flag:comm</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>flag:done</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>let:id = int:1</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>let:writes = string:"x"</string>
            </attr>
        </edge>
        <edge from="n846" to="n846">
            <attr name="label">
                <string>let:writes = string:"y"</string>
            </attr>
        </edge>
        <edge from="n846" to="n908">
            <attr name="label">
                <string>rf</string>
            </attr>
        </edge>
        <edge from="n846" to="n908">
            <attr name="label">
                <string>rt</string>
            </attr>
        </edge>
        <edge from="n878" to="n878">
            <attr name="label">
                <string>type:Tr</string>
            </attr>
        </edge>
        <edge from="n878" to="n878">
            <attr name="label">
                <string>flag:done</string>
            </attr>
        </edge>
        <edge from="n878" to="n878">
            <attr name="label">
                <string>let:id = int:2</string>
            </attr>
        </edge>
        <edge from="n878" to="n878">
            <attr name="label">
                <string>let:reads = string:"x"</string>
            </attr>
        </edge>
        <edge from="n878" to="n878">
            <attr name="label">
                <string>let:reads = string:"y"</string>
            </attr>
        </edge>
        <edge from="n878" to="n908">
            <attr name="label">
                <string>rt</string>
            </attr>
        </edge>
        <edge from="n878" to="n846">
            <attr name="label">
                <string>rw</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>type:Tr</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>flag:comm</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>flag:done</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>let:id = int:3</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>let:reads = string:"x"</string>
            </attr>
        </edge>
        <edge from="n908" to="n908">
            <attr name="label">
                <string>let:reads = string:"y"</string>
            </attr>
        </edge>
    </graph>
</gxl>
